...
export async function POST(request: Request) {
  console.log("Incoming request: POST /api/parse (Next.js)");
  try {
    ...
    if (!apiKey) {
      ...
      return Response.json({
        error: "Parsing Error: AI analysis is temporarily unavailable. Please try again later.",
      }, { status: 500 });
    }
...
export default async function handler(req: any, res: any) {
  console.log("Incoming request: POST /api/parse (Vercel Serverless)");
